...
/**
 * 
 * Interface to an underlying H2 DB containing term time series information for
 * a collection.
 *
 */
public class TimeSeriesIndex {
	...
	public void open(String path, boolean readOnly) throws IOException {
	...
	public double[] getChiSq(String term, double alpha) throws IOException {
		...
		for (int bin = 0; bin < totals.length; bin++) {
			...
			if (alpha > 0) {
				...
				// System.out.println(term + "," + N + "," + counts[bin] + "," +
				// sum + "," + totals[bin]);
				double csq = 0;
				if (counts[bin] > 0 && totals[bin] > 0)
					csq = chiSq(N, (double) counts[bin], sum, (double) totals[bin]);
				// double pval = chiSqTest(N, (double)counts[bin], sum,
				// (double)totals[bin]);
				// ChiSquaredDistribution csdist = new
	...
	private double chiSq(double N, double nX1Y1, double nX1, double nY1) {
		...
		/*
		 * double[] observed = new double[4]; double[] expected = new double[4];
		 * 
		 * observed[0] = nX1Y1; observed[1] = nX1Y0; observed[2] = nX0Y1;
		 * observed[3] = nX0Y0;
		 * 
		 * expected[0] = nY1*nX1/N; expected[1] = nY0*nX1/N; expected[2] =
		 * nY1*nX0/N; expected[3] = nY0*nX0/N;
		 * 
		 * 
		 * double chisq = 0; for (int i=0; i<4; i++) { chisq +=
